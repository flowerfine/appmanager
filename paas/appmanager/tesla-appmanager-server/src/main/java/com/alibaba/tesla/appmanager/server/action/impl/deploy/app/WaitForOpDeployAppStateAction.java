...
/**
 * App 部署工单 State 处理 Action - WAIT_FOR_OP
 *
 * @author yaoxing.gyx@alibaba-inc.com
 */
@Slf4j
@Service("WaitForOpDeployAppStateAction")
public class WaitForOpDeployAppStateAction implements DeployAppStateAction, ApplicationRunner {

    private static final DeployAppStateEnum STATE = DeployAppStateEnum.WAIT_FOR_OP;
    ...
    public void run(ApplicationArguments args) throws Exception {
        timer = meterRegistry.timer("deploy.app.status.waitforop.timer");
        counter = meterRegistry.counter("deploy.app.status.waitforop.counter");
    ...
    public void run(DeployAppDO order, Map<String, String> attrMap) {
        ...
        counter.increment();
        log.info("deploy app has reached waitforop state|deployAppId={}|appPackageId={}|cost={}",
